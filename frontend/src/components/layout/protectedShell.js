...
const CLINIC_SCOPED_ADMIN_SECTIONS = [
  "profile",
  "settings",
  "integrations",
  "staff",
...
const LEGACY_SUPER_ADMIN_CLINIC_PREFIXES = [
  "/clinic-profile",
  "/clinic-settings",
  "/integrations",
  "/staff",
...
const SUPER_ADMIN_GLOBAL_MODE_PATHS = [
  "/dashboard",
  "/clinics",
  "/notifications",
  "/staff-requests",
...
const SUPER_ADMIN_BASE_ALLOWED_PREFIXES = [
  "/dashboard",
  "/clinics",
  "/notifications",
  "/staff-requests",
...
function isLegacyClinicScopedAdminPath(pathname) {
  return LEGACY_SUPER_ADMIN_CLINIC_PREFIXES.some((href) =>
    matchesRoutePrefix(pathname, href)
  );
}

function isNestedClinicScopedAdminPath(pathname) {
  ...
  return CLINIC_SCOPED_ADMIN_SECTIONS.includes(section);
}

function isClinicScopedAdminPath(pathname) {
  return (
    isLegacyClinicScopedAdminPath(pathname) ||
    isNestedClinicScopedAdminPath(pathname)
  );
}

function isPathAllowedForUser(user, pathname) {
  if (!user || !pathname) return false;

  if (user.role === "super_admin") {
    if (
      SUPER_ADMIN_BASE_ALLOWED_PREFIXES.some((href) =>
        matchesRoutePrefix(pathname, href)
      )
    ) {
      return true;
    ...
    if (isClinicScopedAdminPath(pathname)) {
      return true;
    }
  ...
  if (isOwnerLike(user)) {
    return OWNER_ALLOWED_PREFIXES.some((href) =>
      matchesRoutePrefix(pathname, href)
    );
  }

  return RECEPTIONIST_ALLOWED_PREFIXES.some((href) =>
    matchesRoutePrefix(pathname, href)
  );
}

function getSuperAdminGlobalPageTitle(pathname) {
  if (matchesRoutePrefix(pathname, "/clinics")) return "Clinics";
  if (matchesRoutePrefix(pathname, "/staff-requests")) return "Staff Requests";
  if (matchesRoutePrefix(pathname, "/support")) return "Support";
  if (matchesRoutePrefix(pathname, "/notifications")) return "Notifications";
  if (matchesRoutePrefix(pathname, "/dashboard")) return "Platform overview";
  return "Platform overview";
}

function getSuperAdminGlobalPageSubtitle(pathname) {
  if (matchesRoutePrefix(pathname, "/clinics")) {
    return "All clinics workspace";
  }

  if (matchesRoutePrefix(pathname, "/staff-requests")) {
    return "All clinics queue";
  }

  if (matchesRoutePrefix(pathname, "/support")) {
    return "All clinics support queue";
  }

  if (matchesRoutePrefix(pathname, "/notifications")) {
    return "Platform-wide notifications";
  }
...
function getSuperAdminTopbarTitle(pathname, selectedAdminClinic) {
  if (isClinicScopedAdminPath(pathname)) {
    return selectedAdminClinic?.name || "Clinic management";
  }

  return getSuperAdminGlobalPageTitle(pathname);
}

function getSuperAdminTopbarSubtitle(pathname, selectedAdminClinic) {
  if (isNestedClinicScopedAdminPath(pathname)) {
    return selectedAdminClinic?.status
      ? `Clinic management view • ${formatClinicStatus(selectedAdminClinic.status)}`
      : "Clinic management view";
  }

  if (isLegacyClinicScopedAdminPath(pathname)) {
    return selectedAdminClinic?.status
      ? `Selected clinic • ${formatClinicStatus(selectedAdminClinic.status)}`
      : "Selected clinic workspace";
  }

  return getSuperAdminGlobalPageSubtitle(pathname);
}
...
export default function ProtectedShell({ children }) {
  ...
  useEffect(() => {
    ...
    const isAllowed = isPathAllowedForUser(user, pathname);
  ...
  function handleSuperAdminGlobalNavigation(event, href) {
  ...
  function shouldUseGlobalModeNavigation(item) {
    return (
      isSuperAdmin &&
      SUPER_ADMIN_GLOBAL_MODE_PATHS.some((href) =>
        matchesRoutePrefix(item.href, href)
      )
    );
  }
  ...
  const topbarTitle = isSuperAdmin
    ? getSuperAdminTopbarTitle(pathname, selectedAdminClinic)
    : workspaceDisplayName;

  const topbarSubtitle = isSuperAdmin
    ? getSuperAdminTopbarSubtitle(pathname, selectedAdminClinic)
    : getRoleLabel(user.role);

  const userDisplayName = user.fullName || user.email;
  const isClinicScopedAdminPage =
    isSuperAdmin && pathname ? isClinicScopedAdminPath(pathname) : false;
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      onClick={
                        shouldUseGlobalModeNavigation(item)
                          ? (event) =>
                              handleSuperAdminGlobalNavigation(event, item.href)
                          : undefined
                      }
                    >
                      {item.label}
        ...
        {isSuperAdmin ? (
          <div className="page-card soft-card premium-panel admin-focus-card">
            <span className="sidebar-subtitle signed-label">
              Current clinic focus
            </span>

            <strong className="signed-clinic">
              {selectedAdminClinic?.name || "No clinic selected"}
            </strong>

            <span className="muted admin-context-copy">
              {selectedAdminClinic
                ? selectedAdminClinic.status
                  ? `Focused clinic • ${formatClinicStatus(selectedAdminClinic.status)}`
                  : "Focused clinic"
                : "Choose a clinic from Clinics to open clinic-specific admin pages."}
            </span>
            ...
            <Link href="/clinics" className="admin-focus-link">
              Open Clinics
            </Link>
          </div>
        ) : null}
