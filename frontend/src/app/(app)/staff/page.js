...
function buildDeactivateWarningMessage(member, isSuperAdmin) {
  ...
  if (isSuperAdmin) {
    return [
      ...
      "",
      "This will revoke their active access for the selected clinic.",
      "Any currently assigned active leads will become unassigned automatically.",
      "Use Staff Requests for owner-role changes so the approval trail stays intact.",
    ].join("\n");
  }
  ...
  if (user?.role === "super_admin") {
    return {
      ...
      description: selectedAdminClinic?.name
        ...
        : "Review staff for the selected clinic.",
      loadingDescription:
        "Checking selected clinic access and preparing clinic staff data.",
    };
  }
    ...
    try {
      ...
      const payload = await api.patch(`/users/${member.id}/status`, {
        ...
        reason:
          nextStatus === "inactive"
            ? isSuperAdmin
              ? "Deactivated by super admin from clinic staff page."
              : "Deactivated by clinic owner from the staff page."
            : isSuperAdmin
              ? "Reactivated by super admin from clinic staff page."
      ...
      if (nextStatus === "inactive") {
        ...
        setNotice(
          `${member.fullName || member.email} was reactivated successfully.`,
        );
      }
    } catch (err) {
  ...
  if (isSuperAdmin && !targetClinicId) {
    return (
      <PagePlaceholder
        ...
        description="Open Clinics, select the clinic you want to manage, and then open Staff from there."
        points={[
          "Go to Clinics from the sidebar",
          "Choose the clinic you want to manage",
          "Open Staff from the clinic action panel",
        ]}
      />
    );
  }
    ...
    <div className="page stack">
        ...
        <section className="page-card stack-sm">
          <div className="stack-sm">
            ...
            <p className="staff-subtle">
              This page shows staff for the selected clinic. Receptionist access
              can be toggled here, while owner-role changes should continue
              through Staff Requests so the approval trail stays intact.
            </p>
          </div>
      ...
      <section className="metrics-grid">
        ...
        <article className="metric-card">
          ...
          <p className="staff-subtle">
            Quick status breakdown for the visible team.
          </p>
        </article>
      </section>
      ...
      {isLoading ? (
        ...
        <section className="page-card">
          <div className="empty-state">
            No staff members matched the current view.
          </div>
        </section>
      ) : (
                ...
                <div className="stack">
                  <div className="staff-card-header">
                    <div className="stack-sm">
                      <div className="staff-card-topline">
                        <span
                          className={`staff-role-badge ${getRoleTone(member.role)}`}
                        >
                          {humanizeToken(member.role)}
                        </span>

                        <span
                          className={`status-pill ${getStatusTone(member.status)}`}
                        >
                  ...
                  {member.role === "owner" ? (
                    <div className="staff-owner-note">
                      Owner-role changes should continue through Staff Requests
                      so the approval and audit trail remain visible.
                    </div>
                  ) : null}
