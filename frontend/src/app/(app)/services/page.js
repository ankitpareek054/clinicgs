...
export default function ServicesPage() {
  ...
  useEffect(() => {
    if (!isBootstrapping && user && !canUseServicesPage(user) && user.role !== "super_admin") {
  ...
  const loadServices = useCallback(
    async ({ refresh = false } = {}) => {
      if (!user?.clinicId || !canUseServicesPage(user) || showSuperAdminPlaceholder) {
        return;
      }
      ...
      try {
        ...
        const payload = await api.get(`/clinic-services/${user.clinicId}`);
  ...
  useEffect(() => {
    if (!isBootstrapping && user && canUseServicesPage(user) && !showSuperAdminPlaceholder) {
      loadServices();
    }
  }, [isBootstrapping, loadServices, showSuperAdminPlaceholder, user]);
  ...
  async function handleCreateSubmit(event) {
    ...
    try {
      ...
      await api.post(`/clinic-services/${user.clinicId}`, {
  ...
  async function handleEditSubmit(event) {
    ...
    try {
      ...
      await api.patch(`/clinic-services/${user.clinicId}/${editingServiceId}`, {
        serviceName: nextName,
  ...
  async function handleArchive(service) {
    try {
      ...
      await api.delete(`/clinic-services/${user.clinicId}/${service.id}`);
  ...
  async function handleReactivate(service) {
    try {
      ...
      await api.patch(`/clinic-services/${user.clinicId}/${service.id}`, {
        serviceName: service.serviceName,
  ...
  if (isBootstrapping) {
    return (
      <PagePlaceholder
        ...
        description="Checking your session and preparing clinic services."
        points={[
  ...
  if (showSuperAdminPlaceholder) {
    return (
      <PagePlaceholder
        title="Super admin services stay separate"
        description="This page is for clinic owners managing their clinic services. Super admin service tooling should stay in a separate admin workspace."
        points={[
          "Owners manage clinic services here",
          "Super admin can manage services elsewhere",
  ...
  if (!canUseServicesPage(user)) {
    return (
      <PagePlaceholder
        ...
        description="The Services page is currently only available to clinic owners."
        points={[
          "Owners manage clinic service catalog here",
          "Receptionists stay focused on operations",
          "Service management remains owner-controlled",
        ]}
      />
    );
  }
